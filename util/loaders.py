...
#             self.df_train = pd.read_csv(os.path.join(PATH) + "/train.csv")
        ...
        if datasetName in datasetDict.keys():
            ...
            print("here", "PATH_TO_DATA" + "/test.csv")
            self.df_test = pd.read_csv("PATH_TO_DATA" + "/test.csv")
            self.df_train = pd.read_csv("PATH_TO_DATA" + "/train.csv")
            self.df_val = pd.read_csv("PATH_TO_DATA" + "/val.csv")
        self.test = MyData("PATH_TO_DATA", self.df_test, datasetName=datasetName, dset="test", std=0.0, s=s,
                           transformation=transformation)
        self.train = MyData("PATH_TO_DATA", self.df_train, datasetName=datasetName, dset="train", std=std_tr, s=s,
                            transformation=transformation)
        self.val = MyData("PATH_TO_DATA", self.df_val, datasetName=datasetName, dset="val", std=std_tr, s=s,
                            transformation=transformation)
